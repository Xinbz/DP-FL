...
parser.add_argument('-e', '--epochs', type=int, default=80, metavar='NUMBER',
                    help='epochs(default: 200)')
parser.add_argument('-n', '--stack_n', type=int, default=1, metavar='NUMBER',
...
if __name__ == '__main__':
    ...
    public_data = np.load('./data/cifar/public_data_4000.npy')
    public_data = public_data.reshape((public_data.shape[0], 32, 32, 3))
    (x_train1, y_train1), (x_test, y_test) = load_data()
    y_test = keras.utils.to_categorical(y_test, num_classes)
    x_test, public_data = color_preprocessing(x_test, public_data)
    ...
    for count in range(100):
        ...
        train = np.load('./data/cifar/1/data_%d.npy'%count)
        x_train = train[:,:3072]
        ...
        print("== DONE! ==\n== COLOR PREPROCESSING... ==")
        # color preprocessing
        x_train, x_train2 = color_preprocessing(x_train, x_train)
        ...
        save_dir = './models/cifar/1/client_%d' % count
        ...
        # set callback
        cbks = [TensorBoard(log_dir='./resnet_{:d}_{}/client_{:d}/'.format(layers, args.dataset,count), histogram_freq=0),checkpoint,
        ...
        best_net = Model(img_input, output)
        best_net = load_model(filepath)
        soft_label.append(best_net.predict(public_data))
    np.save('./data/cifar/1/soft_label.npy',soft_label)
